...
class lidiform(Form):
    Matej = TextField('Vase jmeno', validators=[
        Predicate(safe_characters, message="Prosim pouzivejte jenom (a-z)"),
        Length(min=1, max=30, message="Maximalne 30 znaku"),
        InputRequired(message="Nesmi zustat prazdne")
    ])
    Leifert = TextField('Vase prijmeni', validators=[
        Predicate(safe_characters, message="Prosim pouzivejte jenom (a-z)"),
        Length(min=1, max=30, message="Maximalne 30 znaku"),
        InputRequired(message="Nesmi zustat prazdne")
    ])
    vek = TextField('Vase datum narozeni ve formatu DD.MM.RRRR', validators=[
            InputRequired(message="Nesmi zustat prazdne")])
